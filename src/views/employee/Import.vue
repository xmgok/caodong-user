  ...
  <div class="goods-import">
    <el-card shadow="never">
      <div slot="header">
        ...
        <a
          :href="`${ siteUrl }/file/employee_template.xlsx`"
          download="员工导入模板.xlsx">
          <el-button
            type="primary"
            size="mini"
      ...
      <div class="upload-wrap">
        <h3 class="mb10 title">请选择员工信息模板：</h3>
        <el-upload
          ref="upload"
          ...
          :headers="{ token }"
          :auto-upload="false"
          :action="`${ postUrl }${ baseUrl }/user/empimport`"
          name="excelFile"
          class="dib mr15"
          multiple>
          <el-button
      ...
      <div class="upload-explain">
        ...
        <ul class="explain-list">
          <li>①下载批量导入模板，并根据模板填写员工信息，需使用xlsx文件上传；</li>
          <li>②填写好需要导入的员工信息，上传文件，并导入；</li>
          <li>③单次上传xlsx最大条目（行数）为1500，超出请按文件分批上传；</li>
          <li>④友情提醒，上传后系统将检查批量员工是否有效，需根据提示进行修改并重新上传xlsx文件</li>
        </ul>
      </div>
    </el-card>
  </div>
...
export default {
  ...
  methods: {
    ...
    submitUpload () {
      this.$confirm(`是否确定导入文件“${this.fileName}”中的员工数据？`, '提示')
