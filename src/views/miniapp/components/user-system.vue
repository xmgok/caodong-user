    ...
    <el-col
      ...
      <div class="content">
        ...
        <div class="item-content">
          <div class="header"><h4 class="sub-title">顶部轮播</h4></div>
          <div class="content-body">
            <div class="img-list clearfix">
              <div
                ...
                <el-card
                  ...
                  shadow="hover"
                  class="img-content">
                  <div @click="openMaterialSelector('banner', index)">
                    <div
                      v-if="item.imgUrl"
                    ...
                    <div
                      ...
                      <p class="img-uploader-tips">建议尺寸为 375 x 170 像素</p>
                    </div>
                  </div>
                  ...
                  <div class="link-address">
                    <div
                      v-if="item.linkName"
                      class="link-wrap">已添加链接至: "{{ item.linkName }}"</div>
                    <el-button
                      round
                      size="mini"
                      @click="openLinkSelector('banner', index)">{{ item.linkName ? '修改' : '添加链接' }}
                    </el-button>
                  </div>
                </el-card>
              </div>
            ...
            <el-button
              ...
              <i class="el-icon-plus"/> {{ bannerList.length ? '继续': '' }}添加
            </el-button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    openMaterialSelector (key, index) {
      this.currentMaterial = key
      this.showMaterialSelector = true
      if (key === 'banner') this.currentMaterialIndex = index
    },
    materialSelectorChanged (data) {
      const imgUrl = data[0].imgUrl || ''
      if (this.currentMaterial !== 'banner') {
        this.imgData[this.currentMaterial].imgUrl = imgUrl
      } else {
        this.bannerList[this.currentMaterialIndex].imgUrl = imgUrl
      }
      this.showMaterialSelector = false
    },
    openLinkSelector (type, index) {
      console.log(type)
      this.currentLink = type
      this.showLinkSelector = true
      if (type === 'banner') this.currentLinkIndex = index
    },
    linkSelectorChanged ({ name, url }) {
      if (this.currentLink !== 'banner') {
        this.imgData[this.currentLink].linkName = name || ''
        this.imgData[this.currentLink].linkUrl = url || ''
      } else {
        this.bannerList[this.currentLinkIndex].linkName = name || ''
        this.bannerList[this.currentLinkIndex].linkUrl = url || ''
      }
      this.currentLink = ''
    },
    handleBannerAdd () {
      this.bannerList.push({ linkUrl: '', linkName: '', imgUrl: '' })
    },
    ...
    getInfo () {
      this.loading = true
      ApiMinapp.getInfo({ appType: 0 }).then(({ data }) => {
        this.loading = false
        if (!data) return

        this.id = data.id || ''
        this.appType = data.appType || 0
        this.bannerList = data.bannerList || []
        const defaultData = { imgUrl: '', isShow: 1, linkName: '', linkUrl: '' }
        const typeList = ['solitaireImg', 'assembleImg', 'sellWellImg']
        ...
        this.imgData.sellWellImg.title = '热销优选'
      })
    },
    submit () {
      const params = {
